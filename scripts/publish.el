...
(defun nyc/embed-video (video-id)
   ...
   (sxml-to-xml
    `(html (@ (lang "en"))
           (head
            ...
            (link (@ (rel "stylesheet") (href ,(concat nyc/site-url "/fonts/jetbrains-mono/jetbrains-mono.css"))))
            (link (@ (rel "stylesheet") (href ,(concat nyc/site-url "/css/code.css"))))
            (link (@ (rel "stylesheet") (href ,(concat nyc/site-url "/css/site.css"))))
            ,(when head-extra head-extra)
            (title ,(concat title " - NYC")))
           (body (@ (class "u-reset")) ,@(unless exclude-header
                                           (nyc/site-header))
                 (div (@ (class "container"))
                      (div (@ (class "site-post flow region"))
...
(defun nyc/publish-newsletter-page (plist filename pub-dir)
...
(defun nyc/format-live-stream-entry (entry style project)
...
(defun nyc/format-news-entry (entry style project)
  "Format posts with author and published data in the index page."
  (cond ((not (directory-name-p entry))
         (format "[[file:%s][%s]] - %s · %s"
                 entry
...
(defun nyc/news-sitemap (title files)
  (format "#+title: %s\n\n%s"
          title
...
(defun nyc/rss-extract-title (html-file)
...
(defun nyc/rss-extract-date (html-file)
  ...
  (with-temp-buffer
    ...
    (let* ((dom (libxml-parse-html-region (point-min) (point-max)))
      ...
      (encode-time 0 0 8 day month year))))
                                        ...
                                        ;(defun nyc/rss-extract-summary (html-file)
                                        ;  )

;; (setq webfeeder-title-function #'nyc/rss-extract-title
;;       webfeeder-date-function #'nyc/rss-extract-date)
...
(setq org-publish-project-alist
      (list '("dofdocs:main"
            ...
            ;;   :sitemap-format-entry nyc/format-live-stream-entry
            ;;   :sitemap-style list
            ...
            ;;   :sitemap-format-entry nyc/format-news-entry
            ;;   :sitemap-style list
            ;;   ;; :sitemap-function nyc/news-sitemap
            ;;   :sitemap-sort-files anti-chronologically
            ;;   :with-title nil
            ;;   :with-timestamps nil)
            ...
            ;;   :publishing-function nyc/publish-newsletter-page)
...
(defun nyc/generate-redirects (redirects)
  (dolist (redirect redirects)
    (let ((output-path (concat "./public/" (car redirect) "/index.html"))
          (redirect-url (concat nyc/site-url "/" (cdr redirect) "/")))
      (make-directory (file-name-directory output-path) t)
      (with-temp-file output-path
        (insert
         (nyc/generate-page "Redirecting..."
...
(defun nyc/publish ()
  ...
  ;; (nyc/generate-redirects '(("support-the-channel" . "how-to-help")
  ;;                          ("videos" . "guides")))
